...
export default function SelectCreateMode({ setViewPage }: { setViewPage: (page: number) => void }) {
  ...
  const createProject = () => {
    if (!user?.id) return;
    submitProjectData(projectData.toCreateProjectDto(user.id));
  };

  return (
    <div className="flex flex-col space-y-4 w-[min(60rem,100%)] px-10 h-[calc(100vh-5rem)] justify-center">
      ...
      <div className="flex w-full gap-10">
        <div className="w-1/3 h-60 border rounded-lg border-gray-300 flex flex-col items-center grow">
          ...
          <button
            className="rounded-md h-10 px-6 relative bottom-0 text-lg bg-gray-200 mt-4"
